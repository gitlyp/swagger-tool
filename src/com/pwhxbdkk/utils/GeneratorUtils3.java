 ...
 */
public class GeneratorUtils3 {
    private static final Logger log = Logger.getInstance(GeneratorUtils3.class);
    ...
    public GeneratorUtils3(Project project, PsiFile psiFile, PsiClass psiClass, String selectionText) {
...
//                    String.format("@ApiImplicitParam(paramType = \"%s\", dataType = \"%s\", name = \"%s\", value = \"\", required = %s)",paramType, dataType, psiParameter.getNameIdentifier().getText());
    ...
    private void generateFieldAnnotation(PsiField psiField){
        ...
        if(docComment == null || StringUtils.isEmpty(docComment.getText())) {
            String apiModelPropertyText = getApiModelPropertyText(psiField);
            if(StringUtils.isNotBlank(apiModelPropertyText)) {
                // 创建注释元素
                PsiComment comment = elementFactory.createCommentFromText("/**\n * " + apiModelPropertyText + "\n */", psiField);
     ...
     * @return
     */
    private String getApiModelPropertyText(PsiField psiField) {
        PsiModifierList modifierList = psiField.getModifierList();
        // 检查字段上是否存在 io.swagger.annotations.ApiModelProperty 注解
